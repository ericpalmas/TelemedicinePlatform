...
import AdminDiseaseListScreen from '../../src/screens/AdminDiseaseListScreen'
...
export const handlers = [
  rest.get('/api/diseases', (req, res, ctx) => {
    return res(
      ctx.json([
        {
...
describe('Testing admin disease list component', () => {
  test('fetches & receives admin disease list ', async () => {
    const history = []
    render(
      <Provider store={store}>
        <BrowserRouter>
          <AdminDiseaseListScreen history={history} />
        </BrowserRouter>
      </Provider>
    )

    expect(await screen.findByText(/Diseases/i)).toBeInTheDocument()
